...
<head>
	...
	<title>FabFlix - Browse by Genre</title>
...
<body>
	...
	<div class="genrelist">
		<ul>
			...
			<c:forEach var="genre" items="${genreList}">
	            <li><a href="browseByGenre?genre=${genre.name}&rpp=${rpp}">&nbsp;${genre.name}</a></li>
        	</c:forEach>
		</ul>
	</div>
			...
			<tr>
				...
				<c:choose>
					<c:when test="${sort == 'TITLE'}">
						<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=TITLE DESC">Title</a></td>
					</c:when>
					<c:otherwise>
						<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=TITLE">Title</a></td>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${sort == 'YEAR'}">
						<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=YEAR DESC">Year</a></td>
					</c:when>
					<c:otherwise>
						<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=YEAR">Year</a></td>
					</c:otherwise>
				</c:choose>
				<td>Director</td>
				<td>Genres</td>
    ...
    <c:if test="${currentPage != 1}">
   		<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=${sort}&page=${currentPage - 1}">Prev</a></td>
	</c:if>
   	<c:if test="${currentPage lt noOfPage}">
   		&nbsp;&nbsp;&nbsp;<td><a href="browseByGenre?genre=${genre}&rpp=${rpp}&sort=${sort}&page=${currentPage + 1}">Next</a></td>
	</c:if>
